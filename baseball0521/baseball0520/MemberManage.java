...
public class MemberManage {
		...
		//private int count; 자동으로 세어지기 때문에 리스트에서는 필요없음
		...
		public MemberManage() {
			...
			dp = new DataProcess("ArrayList0521");
			dp.createFile();
			dataLoad();
			/*
		...
		public void select() {
			...
			int count = 0;
				// 입력된 이름의 선수가 몇명?
					...
					ArrayList<Human>findlist = new ArrayList<Human>();
					//int len = 0;
					for (int i = 0; i < list.size(); i++) {
						//if(list.get(i) != null && !list.get(i).getName().equals("")) {
							if(name.equals(list.get(i).getName())) {
		...
		//TODO 6. 파일에 저장하기
			...
			public void dataLoad() {
				ArrayList<Human> filelist = dp.fileLoad();
				for(int i = 0 ; i<list.size();i++) {
					String data[] =list.get(i).toString().split("-");
					int pos = Integer.parseInt(data[0]);
					if(pos<2000) {
						filelist.add(new Pitcher(Integer.parseInt(data[0]), 
								                                           data[1],
						...
						filelist.add(new Batter(Integer.parseInt(data[0]), 
									                                        data[1],
									                                        Integer.parseInt(data[2]),
									                                        Integer.parseInt(data[3]),
									                                        Integer.parseInt(data[4]),
